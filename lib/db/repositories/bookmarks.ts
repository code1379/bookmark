...
function resolveFallbackCategoryName(userId: number, categoryId: number | null) {
  if (categoryId === null) {
...
export async function listBookmarks(userId: number, limit = 24): Promise<BookmarkListItem[]> {
  if (!isD1Configured()) {
...
async function resolveCategoryIdForCreate(userId: number, input: CreateBookmarkInput) {
...
export async function deleteBookmark(userId: number, id: number) {
  if (!isD1Configured()) {
    const exists = fallbackStore.bookmarks.some((item) => item.id === id && item.userId === userId);
    if (!exists) {
  ...
  const exists = await queryD1("SELECT id FROM bookmarks WHERE user_id = ? AND id = ? LIMIT 1", [
    userId,
    id
  ]);
  if (exists.length === 0) {
  ...
  await queryD1("DELETE FROM bookmarks WHERE user_id = ? AND id = ?", [userId, id]);
}
